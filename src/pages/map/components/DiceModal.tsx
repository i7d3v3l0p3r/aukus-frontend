...
export default function DiceModal({ open, dice, onTurnFinish, onClose }: Props) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} fullWidth keepMounted>
      <DialogTitle>Бросок кубика {diceRollSum && `: ${diceRollSum} (${diceRoll})`}</DialogTitle>
      <DialogContent>
        <div
      ...
      <DialogActions>
        <Button fullWidth onClick={handleFinishTurn} disabled={!isTurnComplete} color="secondary" variant="contained">
          Ходить
        </Button>
      </DialogActions>
    </Dialog>
